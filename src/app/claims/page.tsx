  ...
  return (
    <DashboardLayout>
      <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {/* Filter Box */}
        <Card className="card-neon-glow p-4 rounded-[2rem] border-none">
          <CardContent className="p-4">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="relative flex-1">
                <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                <Input className="pl-14 h-16 font-black bg-white/5 border-white/5 text-white rounded-[1.25rem] focus:border-primary/40 text-lg placeholder:text-white/20" placeholder="Search Global Claim ID..." />
              </div>
              <div className="flex gap-4">
                <Select>
                  <SelectTrigger className="w-[200px] h-16 font-black bg-white/5 border-white/5 text-white rounded-[1.25rem] text-sm">
                ...
                <Button variant="outline" className="h-16 w-16 rounded-[1.25rem] border-white/5 bg-white/5 text-white hover:bg-primary/20 btn-hover-effect">
            ...
            <Table>
              <TableHeader className="bg-white/5">
                <TableRow className="border-white/5 hover:bg-transparent">
                  <TableHead className="px-10 py-6 font-black text-[10px] uppercase tracking-widest text-white/40">Claim Reference</TableHead>
                  <TableHead className="px-10 py-6 font-black text-[10px] uppercase tracking-widest text-white/40">Timestamp</TableHead>
                  <TableHead className="px-10 py-6 font-black text-[10px] uppercase tracking-widest text-white/40">Parametric Trigger</TableHead>
              ...
              <TableBody>
                {claims.map((claim) => (
                  ...
                  >
                    <TableCell className="px-10 py-10 font-mono text-sm font-black text-white">{claim.id}</TableCell>
                    <TableCell className="px-10 py-10 font-black text-white/60 text-sm">{claim.date}</TableCell>
                    <TableCell className="px-10 py-10">
                      <div className="inline-flex px-4 py-2 bg-primary/10 border border-primary/20 rounded-xl">
                        <span className="text-[11px] font-black text-primary uppercase tracking-tight">{claim.trigger}</span>
                      </div>
                    </TableCell>
                    <TableCell className="px-10 py-10 font-black text-white/80">{claim.location}</TableCell>
                    <TableCell className="px-10 py-10">
                      <span className="text-2xl font-black text-primary icon-neon-glow tracking-tighter">{claim.amount}</span>
                    </TableCell>
                    <TableCell className="px-10 py-10">{getStatusBadge(claim.status)}</TableCell>
                ...
                <div className="space-y-8 relative before:absolute before:left-[11px] before:top-4 before:bottom-4 before:w-[2px] before:bg-white/5">
                  {[
                    ...
                    <div key={i} className="flex items-start gap-8 relative z-10">
                      ...
                      <div className="flex-1 bg-white/5 p-5 rounded-2xl border border-white/5 flex justify-between items-center group hover:bg-white/10 transition-colors">
                        <div>
