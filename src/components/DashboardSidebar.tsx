...
export function DashboardSidebar() {
  ...
  return (
    <aside className="w-[260px] min-w-[260px] max-w-[260px] bg-black/95 backdrop-blur-3xl border-r border-primary/20 h-screen flex flex-col sticky top-0 overflow-hidden z-50">
      {/* Logo Section - Precision Branded */}
      ...
      {/* Navigation - Compact & High-Impact Icons */}
      <nav className="flex-1 px-3 space-y-1 py-1 overflow-y-auto no-scrollbar">
        {navItems.map((item) => (
          <Link
            ...
            href={item.href}
            className={cn(
              "flex items-center gap-4 px-4 py-1 rounded-full transition-all duration-300 font-bold group relative overflow-hidden btn-hover-effect",
              pathname === item.href 
      ...
      {/* Footer Section - Compact rounded-full focus */}
      <div className="p-3 mt-auto bg-black/40 border-t border-white/5 shrink-0">
        <div className="flex items-center gap-3 mb-2 p-2.5 bg-white/5 rounded-full border border-white/5 btn-hover-effect cursor-pointer group">
          <Avatar className="w-8 h-8 border border-primary/30 bg-black shadow-[0_0_15px_rgba(0,172,193,0.2)] rounded-full overflow-hidden shrink-0">
        ...
        >
          <div className="p-2 bg-destructive/10 rounded-full flex items-center justify-center">
