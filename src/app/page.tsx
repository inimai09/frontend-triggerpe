...
export default function LandingPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen font-body text-white">
        ...
        <div className="w-full lg:w-[400px] animate-in fade-in zoom-in-95 duration-1000 delay-300">
          <Carousel opts={{ loop: true }} className="w-full cursor-grab active:cursor-grabbing">
            ...
            <div className="flex justify-center gap-4 mt-8">
              <CarouselPrevious className="static translate-y-0 bg-white/5 border-white/10 shadow-lg w-12 h-12 rounded-full text-primary hover:text-primary/80 flex items-center justify-center backdrop-blur-md" />
              <CarouselNext className="static translate-y-0 bg-white/5 border-white/10 shadow-lg w-12 h-12 rounded-full text-primary hover:text-primary/80 flex items-center justify-center backdrop-blur-md" />
            </div>
          </Carousel>
        </div>
      </section>
      ...
      <section className="py-24 px-6 lg:px-24 bg-white/5 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-20 space-y-4 animate-in fade-in slide-in-from-bottom-8 duration-700">
            <h2 className="text-4xl font-black tracking-tighter font-headline text-white uppercase">Three Steps to Safety</h2>
            <div className="w-20 h-1.5 bg-primary mx-auto rounded-full" />
            <p className="text-white/50 font-bold uppercase tracking-widest text-[10px]">The parametric revolution for every partner</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="group relative animate-in fade-in slide-in-from-bottom-8 duration-700 transition-all hover:scale-[1.03] cursor-default">
                <div className="absolute -top-4 -left-4 z-20 w-12 h-12 rounded-full bg-primary flex items-center justify-center font-black text-white text-lg shadow-xl border-4 border-[#001A1A]">
                  {step.step}
                </div>
                <div className="p-8 border-2 border-white/10 rounded-[2.5rem] bg-black/40 backdrop-blur-xl group-hover:border-primary/50 transition-all shadow-2xl h-full flex flex-col items-center md:items-start">
                  <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center border-2 border-white/10 mb-6 shadow-inner group-hover:bg-primary/10 transition-colors">
                    <step.icon className="w-8 h-8 text-primary" />
                  </div>
                  <h3 className="text-2xl font-black font-headline text-white mb-3 uppercase tracking-tighter">{step.title}</h3>
                  <p className="text-base font-medium text-white/60 leading-relaxed text-center md:text-left">{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-24 px-6 lg:px-24">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-20 space-y-4 animate-in fade-in slide-in-from-bottom-8 duration-700">
            <h2 className="text-4xl font-black tracking-tighter font-headline text-white uppercase">Simple Pricing</h2>
            <div className="w-20 h-1.5 bg-primary mx-auto rounded-full" />
            <p className="text-white/50 font-bold uppercase tracking-widest text-[10px]">Transparent protection tiers</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {pricingPlans.map((plan, i) => (
              <Card key={i} className={cn("border-white/10 bg-black/40 backdrop-blur-xl shadow-2xl rounded-[3rem] overflow-hidden flex flex-col transition-all hover:scale-105 duration-300 animate-in fade-in slide-in-from-bottom-12")}>
                <CardContent className="p-10 flex flex-col flex-1">
                  <span className="text-[9px] font-black uppercase text-primary tracking-[0.3em] mb-4">{plan.name}</span>
                  <div 
                  ...
                  >
                    <div className="flex items-baseline gap-1 group-hover:scale-110 transition-transform origin-left">
                      <IndianRupee className="w-5 h-5 text-white mb-1" />
                      <span className="text-5xl font-black text-white tracking-tighter">{plan.price}</span>
                      <span className="text-xs font-bold text-white/50">/week</span>
                    </div>
                    <p className="text-[9px] font-black text-primary uppercase mt-2 opacity-0 group-hover:opacity-100 transition-opacity">Click to Refresh</p>
                  </div>
                  <div className="space-y-4 flex-1">
                    {plan.features.map((feature, idx) => (
                      <div key={idx} className="flex items-center gap-3 text-xs font-bold text-white/80">
                  ...
                  <Button 
                    onClick={handlePriceClick}
                    className="w-full h-14 rounded-full bg-primary hover:bg-primary/90 text-white font-black uppercase tracking-widest text-[10px] mt-10 shadow-lg btn-hover-effect border-none"
                  >
      ...
      <section className="py-24 px-6 lg:px-24 bg-white/5 backdrop-blur-sm">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-20 space-y-4 animate-in fade-in slide-in-from-bottom-8 duration-700">
            <div className="w-14 h-14 bg-white/5 backdrop-blur-md rounded-full flex items-center justify-center mx-auto mb-6 shadow-md border border-white/10">
              <CircleHelp className="w-7 h-7 text-primary" />
            </div>
            <h2 className="text-4xl font-black tracking-tighter font-headline text-white uppercase">Common Questions</h2>
            <div className="w-20 h-1.5 bg-primary mx-auto rounded-full" />
          </div>

          <Card className="bg-black/40 backdrop-blur-xl border-white/10 shadow-2xl rounded-[3rem] p-10 animate-in fade-in slide-in-from-bottom-12 duration-1000">
            <Accordion type="single" collapsible className="w-full">
              {[
                ...
                <AccordionItem key={i} value={`item-${i}`} className="border-white/10 px-2 py-1">
                  <AccordionTrigger className="text-lg font-black text-white hover:no-underline font-headline text-left">
                    {item.q}
                  </AccordionTrigger>
                  <AccordionContent className="text-base font-medium text-white/60 leading-relaxed pt-2">
                    {item.a}
                  </AccordionContent>
                </AccordionItem>
              ))}
      ...
      <footer className="py-16 px-6 lg:px-24 bg-[#001A1A]/80 backdrop-blur-xl text-white">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10">
          <div className="text-center md:text-left">
            <h2 className="text-3xl font-black tracking-tighter font-headline uppercase">TRIGGERPE</h2>
            <p className="text-[9px] font-black text-white/50 uppercase tracking-[0.4em] mt-2">Parametric Shield for Bharat</p>
          </div>
          <div className="flex flex-col items-center md:items-end gap-4">
            <div className="flex gap-8 text-[9px] font-black uppercase tracking-widest text-white/70">
              <Link href="#" className="hover:text-primary transition-colors">Privacy</Link>
              <Link href="#" className="hover:text-primary transition-colors">Terms</Link>
              <Link href="#" className="hover:text-primary transition-colors">Contact</Link>
            </div>
            <p className="text-[10px] font-bold text-white/30 text-center">© 2026 TriggerPe Parametric Insurance. IRDAI Registered Service.</p>
          </div>
        </div>
      </footer>
    </div>
